...
def compute_degree_of_consolidation(Tv: float | np.ndarray) -> float | np.ndarray:
    ...
    """
    is_scalar = np.isscalar(Tv)
    Tv_arr = np.atleast_1d(np.asarray(Tv, dtype=float))
    U = np.zeros_like(Tv_arr)

    mask_pos = Tv_arr > 0
    mask_early = mask_pos & (Tv_arr <= 0.2827)
    mask_late = Tv_arr > 0.2827

    U[mask_early] = np.sqrt(4.0 * Tv_arr[mask_early] / np.pi)
    U[mask_late] = 1.0 - (8.0 / (np.pi**2)) * np.exp(
        -(np.pi**2) * Tv_arr[mask_late] / 4.0
    )
...
def compute_settlement_vs_time(
    ...
    for i, layer in enumerate(profile.layers):
        ...
        if is_clay:
            ...
            Tv = layer.Cv * times_s / (Hdr**2)
            U = compute_degree_of_consolidation(Tv)
            settlements += U * layer_ult[i]
        else:
