...
def fadum_corner_stress(
    ...
    """
    if not isinstance(b, np.ndarray) and not isinstance(z, np.ndarray):
        if z <= 1e-6:
            return 0.25
    ...
    shape = np.broadcast_shapes(np.shape(b), np.shape(z))
    ...
    mask_b_zero = (b_arr <= 1e-6) | (l_dim <= 1e-6)

    mask_calc = ~(mask_z_zero | mask_b_zero)

    out[mask_z_zero] = 0.25
    out[mask_b_zero & ~mask_z_zero] = 0.0
    ...
    if np.any(mask_calc):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            mask_zero = np.abs(diff) < 1e-12
            mask_neg = diff < 0
            mask_pos = ~(mask_zero | mask_neg)

            arg2_val[mask_zero] = np.pi / 2.0
            arg2_val[mask_neg] = np.arctan(arg2[mask_neg]) + np.pi
            arg2_val[mask_pos] = np.arctan(arg2[mask_pos])

            val = (1.0 / (4.0 * np.pi)) * (term1 + arg2_val)
            out[mask_calc] = val
...
def boussinesq_strip_stress(
    ...
    x_arr = np.broadcast_to(x_rel, shape)
    z_arr = np.broadcast_to(z, shape)
    ...
    out[mask_z_zero & (np.abs(x_arr) <= B / 2.0)] = float(q)
    ...
    if np.any(mask_calc):
        ...
        alpha = np.arctan(x_R / zz) - np.arctan(x_L / zz)
...
def boussinesq_rectangular_stress(
    ...
    Iz = np.zeros(shape, dtype=np.float64)

    if np.any(mask_inside):
        b1 = B / 2.0 - abs_x[mask_inside]
        b2 = B / 2.0 + abs_x[mask_inside]
        zz = z_arr[mask_inside]
        Iz[mask_inside] = 2.0 * (
            fadum_corner_stress(b1, y_half, zz) + fadum_corner_stress(b2, y_half, zz)
        )

    if np.any(mask_outside):
        b_far = abs_x[mask_outside] + B / 2.0
        b_near = abs_x[mask_outside] - B / 2.0
        zz = z_arr[mask_outside]
        Iz[mask_outside] = 2.0 * (
            fadum_corner_stress(b_far, y_half, zz)
            - fadum_corner_stress(b_near, y_half, zz)
        )
...
def compute_load_stress_increment(
    ...
    if method == StressMethod.TWO_TO_ONE:
        if load.type == LoadType.STRIP:
            ...
            x_arr = np.broadcast_to(x_rel, shape)
            d_arr = np.broadcast_to(depth, shape)

            out = np.zeros(shape, dtype=np.float64)
            mask = np.abs(x_arr) <= (B + d_arr) / 2.0
            out[mask] = (q * B) / (B + d_arr[mask])
            if out.ndim == 0:
                return float(out)
            return out
        else:
            ...
            out = np.zeros(shape, dtype=np.float64)
            mask = np.abs(x_arr) <= (B + d_arr) / 2.0
            out[mask] = (q * B * L) / ((B + d_arr[mask]) * (L + d_arr[mask]))
            if out.ndim == 0:
                return float(out)
            return out
...
def compute_stress_heatmap(
    ...
    # Ensure empty outputs match expected dimensions
    if len(z_points) == 0 or len(x_points) == 0:
        return np.empty((len(z_points), len(x_points)), dtype=np.float64)
    ...
    for load in loads:
        ...
        increment = compute_load_stress_increment(load, x_rel, Z, method)
        if isinstance(increment, np.ndarray):
            heatmap += increment
        else:
            heatmap += float(increment)

    heatmap /= max(1.0, float(primary_q))
    return heatmap
