...
## 2024-08-16 - Vectorized Stress Heatmap Computation
**Learning:** `compute_stress_heatmap` in `src/settlewell/stress.py` used a double loop to calculate stress over a 2D mesh, creating a bottleneck for dense grid evaluations. Vectorizing the underlying mathematical equations to accept array inputs allowed array operations in `np.meshgrid` format to completely eliminate Python loops.
**Action:** When vectorizing mathematical functions to support both scalar and array inputs, check the dimensionality of the output (e.g., `if out.ndim == 0: return float(out)`) to ensure strict backward compatibility with existing public scalar APIs. When vectorizing functions for performance, broaden type hints (e.g., from `float` to `float | NDArray[np.float64]`) and handle both scalar and array inputs internally to avoid breaking existing public API contracts.
