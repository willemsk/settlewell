...
## Validation & Physics Convergence Specification (v7)

### Generator Script: `scripts/generate_docs_plots.py`
Computes numerical convergence metrics and outputs 4 vector SVG plots to `docs/assets/images/`:
1. `fd_grid_refinement.svg`: 2D Finite-Difference grid refinement ($\Delta x = 10\text{m}, 5\text{m}, 2\text{m}, 1\text{m}, 0.5\text{m}$) compared against Thiem analytical solution.
2. `theis_to_thiem_convergence.svg`: Unsteady Theis drawdown curves ($t = 1\text{d}, 5\text{d}, 20\text{d}, 41.15\text{d}$) converging to steady-state Thiem drawdown.
3. `mesh_independence.svg`: Total 1D primary consolidation settlement vs sublayer discretization $N \in [1, 64]$ demonstrating spatial mesh independence.
4. `cooper_jacob_approximation.svg`: Relative error of Cooper-Jacob logarithmic approximation vs exact Theis $W(u)$ over $u \in [10^{-4}, 10^0]$.

### Documentation Page: `docs/validation.md`
Dedicated page added to site navigation (`Validation & Physics: validation.md`) explaining the 4 limiting cases with theoretical formulas, LaTeX math, GitHub-style alerts, and embedded SVG figures.
