...
def generate_fd_grid_refinement_plot():
    ...
    layer = SoilLayer("Sand", thickness=10.0, gamma=17.5, gamma_sat=20.0, k_h=T / 10.0, e0=0.5, Cc=0.02, Cr=0.005, Eoed=30000, Cv=1e-2)
    ...
    s_analytical = thiem_drawdown_single_well(x_analytical, Q=Q, T=T, R=R_val, H0=H0, aquifer_type=AquiferType.CONFINED)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(x_analytical, s_analytical, color="black", linewidth=2.5, label="Thiem Analytical Solution")
    ...
    for dx, color, marker in zip(dx_list, colors, markers):
        ...
        ax.plot(eval_x, s_num, marker=marker, linestyle="--", color=color, alpha=0.85, label=f"FD Grid (Δx = {dx}m)")

    ax.set_xlabel("Distance from Well r [m]")
    ax.set_ylabel("Drawdown s [m]")
    ax.set_title("2D Finite-Difference Grid Refinement Convergence (Δx → 0)", weight="bold")
    ax.set_xlim(0, 150)
...
def generate_theis_to_thiem_plot():
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(r_vals, s_thiem, color="black", linewidth=2.5, linestyle="-", label=f"Thiem Steady-State (R={R:.0f}m)")
    ...
    for t_d, color in zip(test_days, colors):
        ...
        lbl = f"Theis t = {t_d:.1f} days" if t_d == t_ss_days else f"Theis t = {t_d:.0f} days"
        ax.plot(r_vals, s_theis, color=color, linewidth=1.8, linestyle="--", label=lbl)

    ax.set_xlabel("Distance from Well r [m]")
    ax.set_ylabel("Drawdown s [m]")
    ax.set_title("Unsteady Theis Drawdown Converging to Steady-State Thiem (t → ∞)", weight="bold")
    ax.set_xlim(0, 150)
    ax.set_ylim(bottom=0)
    ax.legend(loc="upper right", frameon=True)
    ax.grid(True, linestyle=":", alpha=0.6)
...
def generate_mesh_independence_plot():
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(n_sub_list, settlements, marker="o", linewidth=2, color="#2980b9", markersize=7, label="1D Primary Settlement")
    ax.axhline(settlements[-1], color="#e74c3c", linestyle="--", linewidth=1.5, label=f"Asymptotic Value ({settlements[-1]:.2f} mm)")
...
def generate_cooper_jacob_plot():
    ...
    ax1.plot(u_vals, w_exact, color="#2c3e50", linewidth=2.2, label="Exact Well Function W(u)")
    ax1.plot(u_vals, w_cj, color="#e67e22", linewidth=2.0, linestyle="--", label="Cooper-Jacob Approx ln(2.25/u)")
    ax1.set_ylabel("Well Function Value [-]")
    ax1.set_title("Cooper-Jacob Logarithmic Approximation (u → 0)", weight="bold")
    ax1.legend(loc="upper right", frameon=True)
    ax1.grid(True, linestyle=":", alpha=0.6)

    ax2.plot(u_vals, rel_error, color="#c0392b", linewidth=2.0, label="Relative Error [%]")
    ax2.axvline(0.01, color="#27ae60", linestyle=":", linewidth=1.8, label="Standard Threshold u = 0.01 (Error ≈ 0.25%)")
